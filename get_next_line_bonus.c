...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/06/30 20:29:26 by khle              #+#    #+#             */
/*   Updated: 2022/07/01 15:26:27 by khle             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (BUFFER_SIZE <= 0 || read(fd, 0, 0) < 0 || fd > 256)
		return (0);
	if (!buff[fd])
	{
	...
	buff[fd] = ft_get_buff(fd, buff[fd]);
	if (!buff[fd])
	{
		free(buff[fd]);
		return (NULL);
